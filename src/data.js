...
// URL → 랜딩구분 역산
export const URL_TO_LANDING = {
  PC: Object.fromEntries(Object.entries(LANDING_URL.PC).map(([k,v]) => [v,k])),
  MO: Object.fromEntries(Object.entries(LANDING_URL.MO).map(([k,v]) => [v,k])),
};
...
export const BUJONG_CFG = {
  ...
  이륜차: { groups: [
    { grp:'이륜차-일반그룹', bj:'보종', kid_pre:'CIR', ptnr_pc:'C614', ptnr_mo:'C614', grp_en_pc:'pcgroup_bike', grp_en_mo:'mogroup_bike', utm_bj:'car_bike_personal' },
  ]},
  법인차: { groups: [
    { grp:'법인차', bj:'보종', kid_pre:'COR', ptnr_pc:'C464', ptnr_mo:'CB03', grp_en_pc:'pcgroup_general_corpatarc', grp_en_mo:'mogroup_corporation', utm_bj:'car_corporate' },
  ]},
  화물차: { groups: [
    { grp:'화물차', bj:'보종', kid_pre:'CIR', ptnr_pc:'C464', ptnr_mo:'C735', grp_en_pc:'pcgroup_buss', grp_en_mo:'general_buss', utm_bj:'car_truck' },
  ]},
  원데이: { groups: [
    { grp:'원데이', bj:'보종', kid_pre:'CIR', ptnr_pc:'C464', ptnr_mo:'C539', grp_en_pc:'pcgroup_oneday', grp_en_mo:'mogroup_oneday', utm_bj:'car_oneday' },
  ]},
  톡보험: { groups: [
    { grp:'톡보험', bj:'보종', kid_pre:'CIS', ptnr_pc:'C464', ptnr_mo:'C465', grp_en_pc:'pcgroup_talk', grp_en_mo:'mogroup_talk', utm_bj:'car_talk' },
  ]},
};
...
export const DEFAULT_LANDING = {
  홈링크:'메인', 브랜드소식:'산출페이지', 메인이미지:'산출페이지', 타이틀:'산출페이지',
  서브링크1:'산출페이지', 서브링크2:'산출페이지', 서브링크3:'산출페이지',
  썸네일1:'산출페이지', 썸네일2:'갱신', 썸네일3:'이륜차', 썸네일4:'산출페이지', 썸네일5:'산출페이지',
  다이나믹섬네일1:'산출페이지', 다이나믹섬네일2:'갱신', 다이나믹섬네일3:'산출페이지',
  다이나믹섬네일4:'산출페이지', 다이나믹섬네일5:'산출페이지',
  메뉴1:'산출페이지', 메뉴2:'산출페이지', 메뉴3:'산출페이지', 메뉴4:'산출페이지',
  '탭1-1':'산출페이지', '탭1-2':'갱신', '탭1-3':'이륜차',
  '탭2-1':'산출페이지', '탭2-2':'산출페이지', '탭2-3':'산출페이지',
  '탭3-1':'산출페이지', '탭3-2':'산출페이지', '탭3-3':'산출페이지',
  컨텐츠1:'산출페이지', 컨텐츠2:'산출페이지', 컨텐츠3:'산출페이지',
  버튼1:'산출페이지', 버튼2:'산출페이지', 버튼3:'산출페이지',
};
...
export function buildRows({ sojae, date, bujong, pcTpl, moTpl, landingPC, landingMO, kidPcStart, kidMoStart }) {
  ...
  // PC 먼저 (구사명범용→신사명범용→구사명보종→신사명보종)
  for (const g of cfg.groups) {
    if (!pcAreas.length) continue;
    for (const area of pcAreas) {
      const landing = landingPC[area] || '산출페이지';
      const baseUrl = LANDING_URL.PC[landing] || LANDING_URL.PC['산출페이지'];
      const kidVal = `${g.kid_pre}${kidPc++}`;
      const areaEn = AREA_EN[area] || area;
      const searchName = `네이버PC_${g.grp}_${g.bj}_${sojae}_${date}_${area}`;
      rows.push({
        dev:'PC', grp:g.grp, bj:g.bj, sojae, date, area, areaEn,
        landing, baseUrl, searchName, kidVal,
        ptnr: g.ptnr_pc, grpEn: g.grp_en_pc, utmBj: g.utm_bj,
      });
    }
  }
  // MO
  for (const g of cfg.groups) {
    ...
    for (const area of moAreas) {
      const landing = landingMO[area] || '산출페이지';
      const baseUrl = LANDING_URL.MO[landing] || LANDING_URL.MO['산출페이지'];
      const kidVal = `${g.kid_pre}${kidMo++}`;
      const areaEn = AREA_EN[area] || area;
      const searchName = `네이버MO_${g.grp}_${g.bj}_${sojae}_${date}_${area}`;
      rows.push({
        dev:'MO', grp:g.grp, bj:g.bj, sojae, date, area, areaEn,
        landing, baseUrl, searchName, kidVal,
        ptnr: g.ptnr_mo, grpEn: g.grp_en_mo, utmBj: g.utm_bj,
      });
    }
  }
  return rows;
